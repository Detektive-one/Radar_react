...
var config = {
  ...
  entries: [], // We will populate this with the CSV data
  // radius: Math.min(screenWidth, screenHeight) / 2.5 // Adjust the radius value based on your preference
};
...
function loadCSV(file) {
  d3.csv(file, function(data) {
    ...
    // Call the radar_visualization function with the updated config
    radar_visualization(config);
  });
}

// Call the loadCSV function with the CSV file path
loadCSV("file.csv");
...
function radar_visualization(config) {
    ...
    const radius = Math.min(screenWidth,screenHeight)/4;
  ...
  const title_offset = { x: -screenWidth / 2 + radius, y: -screenHeight / 2 + radius };
  // const footer_offset = { x: -screenWidth / 2 + radius, y: screenHeight / 2 - radius };

  const legend_offset = [
    { x: screenWidth / 2 - radius * 1.5, y: screenHeight / 2 - radius * 0.8 },
    { x: -screenWidth / 2 + radius * 0.8, y: screenHeight / 2 - radius * 0.8 },
    { x: -screenWidth / 2 + radius * 0.8, y: -screenHeight / 2 + radius * 0.5 },
    { x: screenWidth / 2 - radius * 1.5, y: -screenHeight / 2 + radius * 0.5 }
  ];
